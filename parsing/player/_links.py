...
from parsing.common import BASE, RankingFilter, EventFilter
...
def get_stat_link(
    ...
    if stat == PlayerStat.CLUTCHES:
        requested_link = (f"{BASE}/stats/players/{stat.value}/{self.key}/1on1/{self.name}?startDate={start_time}"
                          f"&endDate={end_time}&matchType={event_fil.value}&rankingFilter={ranking_fil.value}")
    else:
        requested_link = (f"{BASE}/stats/players/{stat.value}/{self.key}/{self.name}?startDate={start_time}"
                          f"&endDate={end_time}&matchType={event_fil.value}&rankingFilter={ranking_fil.value}")
