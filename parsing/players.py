...
from common import BASE, TIMEOUT, EventFilter, FantasyError, set_timeout
...
class Player:
    ...
    def stats_link(self, start: str, end: str, fil: EventFilter) -> str:
        ...
        """
        return (
            f"{BASE}/stats/players/matches/{self.key}/{self.name}?startDate={start}&endDate={end}"
    ...
    def get_events(self, start: dt.date, end: dt.date) -> List[int]:
        ...
        if _src.find("table", class_="stats-table") is not None:
            for block in _src.find("table", class_="stats-table").find_all(
                "img", class_="eventLogo"
            ):
                _event = int(block.find_next_sibling("a").get("href").split("=")[-1])
                if _event != 1040:
                    _events.append(
                        int(block.find_next_sibling("a").get("href").split("=")[-1])
                    )
        return list(set(_events))

    @set_timeout(TIMEOUT)
    def get_stats(
        self, start: dt.date, end: dt.date, fil: EventFilter
    ) -> Tuple[float, float]:
        """
        ...
        _url = self.stats_link(
            start.strftime("%Y-%m-%d"), end.strftime("%Y-%m-%d"), fil
        )
    ...
    @set_timeout(TIMEOUT)
    def get_dataset(
        self, start: dt.date, end: dt.date, delta: dt.timedelta
        ...
        _types = {
            ...
            "event": str,
            "avg rank": np.float32,
            "all events rating": np.float32,
            "big events rating": np.float32,
            "all events wr": np.float32,
            "big events wr": np.float32,
            "pts": np.float32,
        }
        ...
        for _key in _events:
            ...
            try:
                ...
                _all_rating, _all_wr = self.get_stats(
                    _event.start - delta,
                    _event.start - dt.timedelta(1),
                    EventFilter.ALL,
                )
                _big_rating, _big_wr = self.get_stats(
                    _event.start - delta,
                    _event.start - dt.timedelta(1),
                    EventFilter.BIG,
                )
                ...
                print(
                    f"rating: {_all_rating} / {_big_rating} wr: {_all_wr} / {_big_wr}"
                )
                _data.append(
                    np.array(
                        [
